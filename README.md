...
Workflow Example
Intake Officer logs in and registers a new case with suspect and crime details
Case is created with status OPEN and logged to audit trail
Analyst logs in and opens the case to investigate
Analyst catalogs video footage, audio recordings, and photos as evidence
Each evidence item receives a unique hash for integrity verification
Admin performs evidence integrity verification before case submission
Case is advanced to SUBMITTED status, automatically locking all details
If additional investigation is needed, Admin unlocks case for UNDER_INVESTIGATION
Final case closure records all actions in the audit trail
